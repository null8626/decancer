...
public class CuredString {
  static {
    ...
    try {
      ...
      NativeUtils.loadLibraryFromJar("/" + libPrefix + "decancer-" + rustTarget + "." + fileExtension);
    } catch (Throwable err) {
      throw new RuntimeException("[" + rustTarget + "] this operating system (" + osName + ") and/or architecture (" + archName + ") is not supported.");
    }
  }
