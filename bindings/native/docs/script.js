...
function markAPIAsSelected(name) {
  previousMarkedElem = [...document.querySelectorAll('#api')].find(x => x.innerHTML === name)
  previousMarkedElem.style.fontWeight = 'bold'
  elementsToScrollInto.push(previousMarkedElem)
  previousMarkedElem.scrollIntoView()
}
...
window.addEventListener('load', () => {  
  ...
  if (/\/decancer_8h(\.html)?$/.test(new URL(window.location).pathname)) {
    ...
    if (hash.length) {
      ...
      for (const child of document.querySelector('.contents').children) {
        if (child.id === hash) {
          ...
          markAPIAsSelected([...child.childNodes].find(x => x.nodeType === Node.TEXT_NODE).nodeValue.replace(/\(\)?/, ''))
      ...
      if (!foundMatchingHash) {
        const matchingElement = [...document.querySelectorAll('.contents table tbody tr')].find(x => x.id === `r_${hash}`)
    ...
    const stfu = [...document.querySelector('.contents').childNodes].find(n => n.nodeType === Node.TEXT_NODE && n.nodeValue === 'The documentation for this struct was generated from the following file:')
    ...
    for (const child of [...document.querySelector('.contents').children]) {
      if (child.className === 'groupheader' && child.innerHTML === 'Detailed Description') {
        childFlag = true
        child.remove()
