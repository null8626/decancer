...
const [ogBinarySha] = await Promise.all([
  ...
  execute(
    `node ${join(ROOT_DIR, 'scripts', 'decode.mjs')} ${join(
      ROOT_DIR,
      'core',
      'bin',
      'confusables.bin'
...
const [ogJsonSha] = await Promise.all([
  ...
  execute(
    `node ${join(ROOT_DIR, 'scripts', 'encode.mjs')} ${join(
      ROOT_DIR,
      'output.json'
    )}`,
    { cwd: ROOT_DIR }
...
if (ogBinarySha !== newBinarySha) {
  console.error(
    `- error: different binary sha256sum detected - the binary must have corruption like your country\n- original: ${ogBinarySha}\n- new: ${newBinarySha}`
  )
  process.exit(1)
}
...
if (ogJsonSha !== newBinarySha) {
  console.error(
    `- error: different json sha256sum detected - the binary must have corruption like your country\n- original: ${ogJsonSha}\n- new: ${newJsonSha}`
  )
  process.exit(1)
}
