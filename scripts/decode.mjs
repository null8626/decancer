...
let bytes = binary.slice(binary.readUint16LE(), binary.readUint16LE(2))
const similar = []

while (bytes.length !== 0) {
  const length = bytes.readUint8()
  similar.push([...bytes.slice(1, 1 + length)].map(x => String.fromCharCode(x)))
  
  bytes = bytes.slice(1 + length)
}

bytes = binary.slice(binary.readUint16LE(2))
const strings = []

while (bytes.length !== 0) {
  const length = bytes.readUint8()
  strings.push([...bytes.slice(1, 1 + length)].reduce((a, b) => a + String.fromCharCode(b), ""))
...
for (let offset = 4; offset < confusablesEnd; offset += 5) {
  ...
  confusables.push({
    codepoint,
    translation: ((integer & 0x20000000) != 0) ? strings[translationCode] : String.fromCharCode(translationCode),
    caseSensitive: (integer & 0x40000000) != 0,
    rangeUntil: ((integer & 0x10000000) != 0) ? (codepoint + (secondByte & 0x7f)) : null,
