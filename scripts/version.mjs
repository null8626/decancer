...
const update = (filename, callback) => {
  writeFileSync(filename, callback(readFileSync(filename).toString()))
  console.log(`- updated file: ${filename}`)
}

const updateTomlFunc = x => x.replace(/version = "\d\.\d\.\d"/, `version = "${process.argv[2]}"`)
...
update(join(ROOT_DIR, 'README.md'), x => x.replace(/(\d\.\d\.\d)/g, process.argv[2]))