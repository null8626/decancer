...
function updateNativeHeaderFunc(x) {
  const versionHex = `0x${process.argv[2].split('.').map(x => x.padStart(2, '0')).join('')}`
  
  return x.replace(/#define DECANCER_VERSION 0x[a-fA-F0-9]{6}/, `#define DECANCER_VERSION ${versionHex}`)
}
...
void (await Promise.all([
  ...
  update(join(ROOT_DIR, 'bindings', 'native', 'decancer.h'), updateNativeHeaderFunc),
